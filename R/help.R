...
#'\deqn{\mathop{min}_{f_\mathcal{g} \in \mathcal{F}} \mathop{min}_{\{\mathbf{z}_1, ..., \mathbf{z}_M\}} \sum_{(V_i, V_j) \in \mathcal{E}} b_{i,j}||f_g(\mathbf{z}_i) - f_g(\mathbf{z}_j)||^2} where
#'\eqn{f_g} is a function to map the instrinsic data space \eqn{\mathcal{Z} = {\mathbf{z}_1, ..., \mathbf{z}_M}} back to the input data space (reverse embedding) \eqn{\cal{X} =
#'\{\mathbf{x}_1, ..., \mathbf{x}_N\}}. \eqn{V_i} is the the vertex of the instrinsic undirected graph \eqn{\mathcal{G} = (\mathcal{V}, \mathcal{E})}. \eqn{\mathcal{E}} is
...
#'\deqn{min_{\mathcal{G} \in \hat{\mathcal{G}}_b}min_{f_\mathcal{g} \in F} min_{\{\mathbf{z}_1, ..., \mathbf{z}_M\}} \sum_{i = 1}^N ||\mathbf{x}_i - f_\mathcal{g}(\mathbf{z}_i)||^2 + \frac{\lambda}{2} \sum_{(V_i, V_j) \in \mathcal{E}}
#'b_{i,j}||f_\mathcal{g}(\mathbf{z}_i) - f_\mathcal{g}(\mathbf{z}_j)||^2} where \eqn{\lambda} is a non-negative parameter which controls the tradeoff between the data
...
#'graph structure, which can be solved by Kruskal' algoritm. A linear projection model \eqn{f_\mathcal{g}(\mathcal{z}) = \mathbf{Wz}} is used as the mapping function.
#'Those setting results in the following specific form for the previous framework:
#'\deqn{\mathop{min}_{\mathbf{W}, \mathbf{Z}, \mathbf{B}} \sum_{i = 1}^N ||\mathbf{x}_i  - \mathbf{W}_{z_i}||^2 + \frac{\lambda}{2} ] \sum_{i,j}b_{i,j}||\mathbf{W}_{z_i} - \mathbf{W}_{\mathbf{z}_j}||^2}
...
#' \deqn{\mathop{min}_{\mathbf{W}, \mathbf{Z}, \mathbf{B}, \mathbf{Y}, \mathbf{R}} \sum_{i = 1}^N ||\mathbf{x}_i - \mathbf{W}_{\mathbf{z}_i}||^2 +
#' \frac{\lambda}{2} \sum_{k, k'}b_{k, k'}||\mathbf{W}_{\mathbf{y}_k} - \mathbf{W}_{\mathbf{y}_k'}||^2 +
#' \lambda[\sum_{k = 1}^K \sum_{i = 1}^N r_{i, k} ||z_i - y_k||^2 + \sigma \Omega (\mathbf{R})]}
#' \deqn{s.t.\ \mathbf{W}^T \mathbf{W} = \mathbf{I}, \mathbf{B} \in \mathbf{B}, \sum_{k = 1}^K r_{i, k} = 1,
#' r_{i, k} \leq 0, \forall i, \forall k} where \eqn{\mathbf{R} \in \mathbf{R}^{N \times N}, \Omega(R) = \sum_{i = 1}^N \sum_{k = 1}^k r_{i, k} log r_{i, k}} is the negative
...
#'1. \eqn{\mathbf{Input}}: Data matrix \eqn{\mathbf{X}}, parameters \eqn{\lambda, \sigma, \gamma} \cr
#'2. Initialize \eqn{\mathbf{Z}} by PCA \cr
#'3. \eqn{K = N, \mathbf{Y} = \mathbf{Z}} \cr
#'4. \eqn{\mathbf{repeat}}: \cr
#'      5.  \eqn{d_{k,k'} = ||\mathbf{y}_k - \mathbf{y}_{k'}||^2, \forall k, \forall k'} \cr
#'      6.   Obtain \eqn{\mathbf{B}} via Kruskal's algorithm \cr
#'      7.   \eqn{\mathbf{L} = diag(\mathbf{B1}) - \mathbf{B}} \cr
#'      8.   Compute \eqn{\mathbf{R}} with each element \cr
#'      9.   \eqn{\tau = diag(\mathbf{1}^T \mathbf{R})} \cr
#'      10.  \eqn{\mathbf{Q} = \frac{1}{\mathbf{1} + \gamma} [\mathbf{I} + \mathbf{R} (\frac{1 + \gamma}{\gamma}(\frac{\lambda}{\gamma} \mathbf{L} +
#'     \tau) - \mathbf{R}^T \mathbf{R})^{-1} \mathbf{R}^T]} \cr
#'      11.  \eqn{\mathbf{C} = \mathbf{X Q X}^T} \cr
#'      12.  Perform eigen-decomposition on \eqn{\mathbf{C}} such that \eqn{\mathbf{C} = \mathbf{U} \wedge \mathbf{U}^T} and \eqn{diag(\wedge)} is sorted in a descending order \cr
#'      13. \eqn{\mathbf{W} = \mathbf{U}(:, 1:d)} \cr
#'      14. \eqn{\mathbf{Z} = \mathbf{W}^T \mathbf{X Q}} \cr
#'      15. \eqn{\mathbf{Y} = \mathbf{Z R} (\frac{\lambda}{\gamma} \mathbf{L} + \tau)^{-1}} \cr
#'16. \eqn{\mathbf{Until}} Convergence \cr
#'
