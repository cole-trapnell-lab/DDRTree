...
test_that("DDRTRee() perform the DDRTree construction", {
    ...
    baseLoc <- system.file(package="DDRTree")
    #baseLoc <- './inst'
    ...
    params <- list(maxIter = 20, eps = 1e-3, dim = 2, lambda = 2435, sigma = 1e-3, gamma = 10, verbose=T)
...
#
    #Cell data:
    extPath <- file.path(baseLoc, "extdata")
    valid_subset_GSE72857_exprs <- readMat(file.path(extPath, "valid_subset_GSE72857.mat"))
